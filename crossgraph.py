...
class MLPClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # 将输入转置为 [seq_len, batch_size, embed_dim]
        x = x.permute(1, 0, 2)
        
        # 多头注意力机制
        attn_output, _ = self.MLP_multihead_attn(x, x, x)
        x = attn_output  # 直接使用注意力输出
        
        # 转置回 [batch_size, seq_len, embed_dim]
        x = x.permute(1, 0, 2)
...
def evaluate_model(classifier, embeddings, labels, threshold=0.5):
    classifier.eval()
    with torch.no_grad():
        embeddings = clean_data(embeddings)  # 清理 embeddings 中的 NaN 和 Inf
...
def save_metrics(domain, auroc, precision, recall, f1, trial_number, is_best, save_folder='optuna_metrics'):
    ...
    if os.path.exists(filename):
        ...
        if is_best:
            ...
            print(f"新的最高 AUROC 指标已保存到 {filename}")
        else:
            print(f"当前 AUROC ({auroc}) 不高于已有记录，未保存。")
    else:
        metrics_df.to_csv(filename, index=False)
...
def objective(trial):
    ...
    # 创建 DataLoader，并设置 num_workers 参数为 CPU 核心数
    ...
    for epoch in range(total_epochs):
        ...
        if total_loss < best_loss:
            ...
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),  # 通用模型保存，不再使用gcn_model
    ...
    is_source_best = auroc_source > 0.5  # 这里改为适当的逻辑来判断最优目标域
    save_metrics("Source", auroc_source, precision_source, recall_source, f1_source, trial.number, True)
    save_predictions_to_h5ad(target_predictions, args.scRNA_dataset, args.Drug)
